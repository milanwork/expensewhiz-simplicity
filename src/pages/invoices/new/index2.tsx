...
export default function NewInvoice2() {
    ...
    const initialize = async () => {
      try {
        ...
        if (editInvoiceData) {
          ...
          // Initialize balance due from existing data
          const total = invoiceData.total || 0;
          const paid = invoiceData.amount_paid || 0;
          setBalanceDue(total - paid);
  ...
  useEffect(() => {
    const { total, balanceDue: newBalance } = calculateTotals();
  ...
  const handleSubmit = async () => {
    ...
    // Define invoice data with calculated totals
    const invoiceData = {
    ...
    if (existingInvoiceId) {
      if (invoiceStatus === 'paid' && totals.balanceDue > 0) {
    ...
    await saveInvoice(totals);
  };

  const saveInvoice = async (totals: ReturnType<typeof calculateTotals>) => {
    setIsLoading(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("Not authenticated");
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold">
              New Invoice (Page 2) {invoiceNumber}
            </h1>
